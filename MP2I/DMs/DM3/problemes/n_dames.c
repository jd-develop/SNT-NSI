...
// Renvoie la formule de contrainte sur la ligne i pour un échiquier de n lignes
char* contrainte_une_ligne(int i, int n){
    ...
    char* f1 = au_moins_une_charetoile(l, n);
    char* f2 = au_plus_une_charetoile(l, n);
    sprintf(f, "(%s & %s)", f1, f2);
    free(f1);
    free(f2);
    for (int i = 0; i < n; i++){
...
/* Renvoie la formule de contrainte sur la colonne j pour un échiquier de n
 * lignes */
char* contrainte_une_colonne(int j, int n){
    ...
    char* f = au_plus_une_charetoile(l, n);
    for (int i = 0; i < n; i++){
        free(l[i]);
    }
    free(l);
...
/*
 * Renvoie la formule de contrainte sur la diagonale commencant à (i, j) avec s
 * le sens de la diagonale (1 pour lignes montantes -1 pour descandantes),
 * pour un échiquier de n lignes
*/
char* contrainte_une_diag(int i, int j, int s, int n){
    ...
    char* f = au_plus_une_charetoile(l, nv);
    for (int i = 0; i < nv; i++){
        free(l[i]);
    }
    free(l);
...
int main (int argc, char** argv){
    if (argc != 2) {
        perror("Ce programme prend un argument : la taille de l'échiquier n");
    } else {
        int n = atoi(argv[1]);
